...
const itActsAsFavoriteRestoModel = (favoriteResto) => {
  ...
  it('should be able to search for resto', async () => {
    favoriteResto.putResto({ id: 1, title: 'makanan a' });
    favoriteResto.putResto({ id: 2, title: 'makanan b' });
    favoriteResto.putResto({ id: 3, title: 'makanan abc' });
    favoriteResto.putResto({ id: 4, title: 'ini mah makanan abcd' });

    expect(await favoriteResto.searchResto('makanan a')).toEqual([
      { id: 1, title: 'makanan a' },
      { id: 3, title: 'makanan abc' },
      { id: 4, title: 'ini mah makanan abcd' },
    ]);
  });
};
